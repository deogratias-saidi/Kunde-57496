...
param parAzBastionName string = toLower('Bastion-${parCompanyPrefix}-HUB')
...
param parCorpSubscriptionId string
...
param parPeerHubId string = '${parCompanyPrefix}-hub-id'
param ParPeerIdHub string = '${parCompanyPrefix}-id-hub'
...
module modSpokeResourceGroup '../../custom-modules/resourceGroup/resourceGroup.bicep' = {
  scope: subscription(parCorpSubscriptionId)
...
module modBastionPublicIP '../../custom-modules/network/publicIp/publicIp.bicep' = if(parAzBastionEnabled) {
  ...
  params:{
    parLandingZoneEnv: 'Connectivity'
    parPublicIpName: toLower('${parPublicIpSuffix}-${parAzBastionName}')
    parPublicIpSku: {
      name: parPublicIpSku
    }
...
module modDcNsg '../../custom-modules/network/networkSecurityGroup/networkSecurityGroup.bicep' = {
  scope: resourceGroup(parCorpSubscriptionId, parSpokeResourceGroupName)
  name: 'dc-nsg'
  params:{
    parLocation: parLocation
    parLandingZoneEnv: 'Corp'
...
module modHubVnet '../../custom-modules/network/virtualNetwork/hubVirtualNetworkAzureFirewall.bicep' = {
  ...
  params:{
    ...
    parAzBastionEnabled: parAzBastionEnabled
    parAzBastionNsgName: modBastionNsg.name
    parLandingZoneEnv: 'Connectivity'
  }
  dependsOn:[
    modHubResourceGroup
...
module modAzureFirewallMgmtPublicIp '../../custom-modules/network/publicIp/publicIp.bicep' = {
  ...
  params: {
    parLandingZoneEnv: 'Connectivity'
    parPublicIpName: toLower('${parAzFirewallName}-MGMT-${parPublicIpSuffix}')
    parPublicIpProperties: {
      publicIpAddressVersion: 'IPv4'
      publicIpAllocationMethod: 'Static'
    }
...
module modAzureFirewallPublicIp '../../custom-modules/network/publicIp/publicIp.bicep' = if (parAzFirewallEnabled) {
  ...
  params: {
    ...
    parPublicIpName: toLower('${parPublicIpSuffix}-FW-${parCompanyPrefix}-CONN')
    parPublicIpProperties: {
      publicIpAddressVersion: 'IPv4'
      publicIpAllocationMethod: 'Static'
    }
...
module modGatewayPublicIpId '../../custom-modules/network/publicIp/publicIp.bicep' = if (parVpnGatewayEnabled) {
  scope: resourceGroup(parConnectivitySubscriptionId, parHubResourceGroupName)
  name: 'deploy-Gateway-Public-IP'
  params: {
    parLocation: parLocation
    parAvailabilityZones: parAzFirewallAvailabilityZones
    parPublicIpName: toLower('${parPublicIpSuffix}-GW-${parCompanyPrefix}-CONN')
    parPublicIpProperties: {
      publicIpAddressVersion: 'IPv4'
      publicIpAllocationMethod: 'Static'
    }
...
module modAzureFirewall '../../custom-modules/network/hubAndSpoke//azureFirewallStandard/azyreFirewallStandard.bicep' = {
  scope: resourceGroup(parConnectivitySubscriptionId, parHubResourceGroupName)
  name: 'AzureFirewall'
  params: {
...
module modGateway '../../custom-modules/network/gateway/newGateway.bicep' = {
  ...
  name: 'gateway'
  params:{
    parGatewayPublicIpId: modGatewayPublicIpId.outputs.outPublicIpId
